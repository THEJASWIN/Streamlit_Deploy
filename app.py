...
def predict(open_ip, high_ip, low_ip, day_ip, month_ip, year_ip, time_hr_ip, time_min_ip):
    our_pred = model1.predict(
        [[open_ip, high_ip, low_ip, day_ip, month_ip, year_ip, time_hr_ip, time_min_ip]])
    print(our_pred)
    return our_pred
...
def main():
    ...
    if st.button("Predict"):
        result = predict(open_ip, high_ip, low_ip, day_ip,
                         month_ip, year_ip, time_hr_ip, time_min_ip)
        print()
